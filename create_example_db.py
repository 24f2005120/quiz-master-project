...
def add_dummy_data():
    ...
    quiz1_algebra = Quiz(quiz_name='Algebra Basics Quiz', chapter_id=chapter1_math.chapter_id, subject_id=chapter1_math.subject_id, date=date(2024, 1, 20), duration=1, remarks='Basic algebra quiz', total_marks=20)
    quiz2_algebra = Quiz(quiz_name='Advanced Algebra Quiz', chapter_id=chapter1_math.chapter_id, subject_id=chapter1_math.subject_id, date=date(2024, 1, 25), duration=45, remarks='Advanced algebra quiz', total_marks=30)
    db.session.add_all([quiz1_algebra, quiz2_algebra])
    db.session.commit()

    # Quizzes for Geometry (Mathematics)
    quiz1_geometry = Quiz(quiz_name='Geometry Fundamentals', chapter_id=chapter2_math.chapter_id, subject_id=chapter2_math.subject_id, date=date(2024, 2, 1), duration=40, remarks='Basic geometry quiz', total_marks=25)
    db.session.add_all([quiz1_geometry])
    db.session.commit()

    # Quizzes for Physics (Science)
    quiz1_physics = Quiz(quiz_name='Newtonian Mechanics', chapter_id=chapter1_science.chapter_id, subject_id=chapter1_science.subject_id, date=date(2024, 2, 10), duration=35, remarks='Basic physics principles', total_marks=20)
    quiz2_physics = Quiz(quiz_name='Thermodynamics', chapter_id=chapter1_science.chapter_id, subject_id=chapter1_science.subject_id, date=date(2024, 2, 15), duration=50, remarks='Heat and energy transfer', total_marks=30)
    db.session.add_all([quiz1_physics, quiz2_physics])
    db.session.commit()

    # Quizzes for Biology (Science)
    quiz1_biology = Quiz(quiz_name='Cell Biology', chapter_id=chapter2_science.chapter_id, subject_id=chapter2_science.subject_id, date=date(2024, 2, 20), duration=40, remarks='Basic cell structures', total_marks=25)
    db.session.add_all([quiz1_biology])
    db.session.commit()

    # Quizzes for Ancient History (History)
    quiz1_ancient_history = Quiz(quiz_name='Ancient Egypt', chapter_id=chapter1_history.chapter_id, subject_id=chapter1_history.subject_id, date=date(2024, 3, 1), duration=45, remarks='Egyptian civilization', total_marks=30)
    db.session.add_all([quiz1_ancient_history])
    db.session.commit()

    # Quizzes for Modern History (History)
    quiz1_modern_history = Quiz(quiz_name='World War I', chapter_id=chapter2_history.chapter_id, subject_id=chapter2_history.subject_id, date=date(2024, 3, 10), duration=50, remarks='Causes and effects of WWI', total_marks=35)
    db.session.add_all([quiz1_modern_history])
    db.session.commit()

    # Quizzes for Poetry (Literature)
    quiz1_poetry = Quiz(quiz_name='Shakespearean Sonnets', chapter_id=chapter1_literature.chapter_id, subject_id=chapter1_literature.subject_id, date=date(2024, 3, 15), duration=40, remarks='Analysis of sonnets', total_marks=25)
    db.session.add_all([quiz1_poetry])
    db.session.commit()

    # Quizzes for Drama (Literature)
    quiz1_drama = Quiz(quiz_name='Hamlet', chapter_id=chapter2_literature.chapter_id, subject_id=chapter2_literature.subject_id, date=date(2024, 3, 20), duration=55, remarks='Shakespearean tragedy', total_marks=40)
